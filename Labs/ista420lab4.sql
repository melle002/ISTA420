...
(select distinct o.empid from sales.orders o where orderdate >= '2016-05-01');
...
where  c.custid = so.custid and so.orderdate >= '2015-01-01' and so.orderdate < '2016-01-01')
and not exists
(select so1.custid from sales.orders so1 
where c.custid = so1.custid and so1.orderdate >= '2016-01-01' and so1.orderdate < '2017-01-01');  
...
--9. Write a query that returns for each order the number of days that passed since the same customer’s previous order. To determine recency among orders, use orderdate as the primary sort element and orderid as the tiebreaker:
...
--10. Write a query that returns for each order the number of days that passed since the same customer’s previous order. To determine recency among orders, use orderdate as the primary sort element and orderid as the tiebreaker:
 --Table involved: Sales.Orders
 --Desired output:
